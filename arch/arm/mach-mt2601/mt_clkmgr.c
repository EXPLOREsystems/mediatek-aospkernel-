...
static int mfg_cg_grp_dump_regs_op(struct cg_grp *grp, unsigned int *ptr)
{
	...
	if (NULL != clk && PWR_ON == get_clk_state_locked(clk)	/* XXX: clock_is_on() locked version */
	    ) {
		*(ptr) = clk_readl(grp->sta_addr);
	} else {
...
static int spm_mtcmos_ctrl_general_locked(struct subsys *sys, int state)
{
	...
	if (state == STA_POWER_DOWN) {
		...
		if (0 != sys->bus_prot_mask) {
			spm_write(INFRA_TOPAXI_PROTECTEN,
				  spm_read(INFRA_TOPAXI_PROTECTEN) | sys->bus_prot_mask);
		...
		while (sys->sram_pdn_ack_bits && ((spm_read(sys->ctl_addr) & sys->sram_pdn_ack_bits) != sys->sram_pdn_ack_bits)	/* wait until SRAM_PDN_ACK all 1 */
		    ) {
			if (time_after(jiffies, expired)) {
				WARN_ON(1);
				break;
 ...
 || !(spm_read(SPM_PWR_STATUS_S) & sys->sta_mask)
		...
		while (sys->sram_pdn_ack_bits && (spm_read(sys->ctl_addr) & sys->sram_pdn_ack_bits)	/* wait until SRAM_PDN_ACK all 0 */
		    ) {
			if (time_after(jiffies, expired)) {
				WARN_ON(1);
				break;
...
#if 0
static int udelay_test_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	char *p = page;
	int len = 0;
...
static int udelay_test_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char desc[32];
	int len = 0;
...
static int golden_test_read(struct seq_file *m, void *v)
{
	...
	static off_t page_len;
	static off_t used_off;
...
#if 0
static int golden_test_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	static int buf_golden_setting_idx;
	static off_t page_len;
	static off_t used_off;
					...
					if (MODE_NORMAL == _golden.mode
						...
						if (MODE_COLOR == _golden.mode) {
							p += sprintf(p,
								     ...
								     HEX_FMT "\t%s\n",
								     _golden.
								     buf_golden_setting[i].addr,
								     _golden.
								     buf_golden_setting[i].mask,
								     snapshot->reg_val[i],
								     _gen_color_str
								     (_golden.buf_golden_setting[i].
								      mask,
...
static int golden_test_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char desc[256];
	int len = 0;
