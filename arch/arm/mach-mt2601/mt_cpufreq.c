...
void mt_cpufreq_disable(unsigned int type, bool disabled)
{
	...
	if (type == 0) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
			    "mt_cpufreq_disable, mt_cpufreq_disabled_by_sdio_autoK = %d\n",
...
void mt_cpufreq_volt_set_by_sdio(unsigned int volt)
{
	...
	if (mt_cpufreq_disabled_by_sdio_autoK == true) {
		...
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
			    "mt_cpufreq_volt_set_by_sdio, could not set volt now,  mt_cpufreq_disabled_by_sdio_autoK = %d\n",
			    mt_cpufreq_disabled_by_sdio_autoK);
	}
#endif
}
...
void restore_default_volt(void)
{
	...
	for (i = 0; i < MAX_SPM_PMIC_TBL; i++) {
		...
		if (spm_pmic_config[i].cpufreq_khz > NOR_MAX_FREQ) {
			...
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "PTPOD: restore entry[], volt[%d]\n", i,
				    spm_pmic_config[i].cpufreq_khz);
			mt65xx_reg_sync_writel(VOLT_TO_PMIC_VAL(spm_pmic_config[i].cpufreq_volt),
	...
	for (i = 0; i < MAX_SPM_PMIC_TBL; i++) {
		if (spm_pmic_config[i].cpufreq_khz == freq) {
			...
			case 5:
				...
				break;
			default:
				dprintk("There is no entry [%d] in spm table for ptpod",
			...
			dprintk("Set spm_pmic_config[%d].cpufreq_volt = %d", i,
	...
	for (i = 0; i < g_cpufreq_power_tbl_num; i++) {
		if ((mt_cpu_power[i].cpufreq_khz == mt_cpu_freqs[idx].cpufreq_khz)
			...
			if (mt_cpu_power[i].cpufreq_power >
				...
				dprintk("%d > %d", mt_cpu_power[i].cpufreq_power,
					mt_cpu_power[g_max_power_OPP].cpufreq_power);
...
void mt_dvfs_power_dispatch_safe(void)
{
	if (!mutex_trylock(&power_mutex)) {
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
			    "@%s: power dispatching, skip this round!\n", __func__);
		return;
	}
    ...
    *************************************************/
	 ...
	 *******************************/
		...
		if (g_limited_power != 0) {
			...
			if (new_OPP_idx < 0) {
				xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
					    "@%s: skip this round due to CPU off not complete!\n",
					    __func__);
				mutex_unlock(&power_mutex);
				return 0;
			}
...
static int mt_cpufreq_power_dump_read(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < MAX_SPM_PMIC_TBL; i++) {
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "spm_pmic_config[%d].tbl_idx = %d\n", i,
			    spm_pmic_config[i].tbl_idx);
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
			    "spm_pmic_config[%d].cpufreq_khz = %d\n", i,
			    spm_pmic_config[i].cpufreq_khz);
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
			    "spm_pmic_config[%d].cpufreq_volt = %d\n", i,
			    spm_pmic_config[i].cpufreq_volt);
	}
	for (i = 0; i < mt_cpu_freqs_num; i++) {
				      ...
				      void *data)
{
	...
	if (off > 0) {
		*eof = 1;
		...
		for (i = 0; i < (g_cpufreq_power_tbl_num); i++) {
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "mt_cpu_power[%d].cpufreq_khz = %d\n", i,
				    mt_cpu_power[i].cpufreq_khz);
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "mt_cpu_power[%d].cpufreq_ncpu = %d\n", i,
				    mt_cpu_power[i].cpufreq_ncpu);
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "mt_cpu_power[%d].cpufreq_power = %d\n", i,
				    mt_cpu_power[i].cpufreq_power);
		}
		pwrap_read(VPROC_VOSEL_CTRL, &pmic_setting);
		xlog_printk(ANDROID_LOG_INFO, "Power/DVFS", "VPROC_VOSEL_CTRL(0x%x) = %d\n",
		...
		for (i = 0; i < mt_cpu_freqs_num; i++) {
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "mt_cpu_freqs[%d].cpufreq_khz = %d\n", i,
				    mt_cpu_freqs[i].cpufreq_khz);
			xlog_printk(ANDROID_LOG_INFO, "Power/DVFS",
				    "mt_cpu_freqs[%d].cpufreq_volt = %d\n", i,
