...
wait:
	...
	if (with_timeout) {
		...
		if (IS_TMO(t)) {
			TPD_DMESG( "%s: tmo waiting exclusive access\n",
				__func__);
			return -ETIME;
		}
	} else {
		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	...
	rc = cyttsp5_adap_write_read_specific(cd, cmd_length, cmd,
			hid_cmd->read_buf);
	if (rc)
		TPD_DMESG( "%s: Fail cyttsp5_adap_transfer\n", __func__);
		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	...
	if (IS_TMO(t)) {
		TPD_DMESG( "%s: HID output cmd execution timed out\n",
			__func__);
		rc = -ETIME;
		goto error;
	}
		...
		u8 power_state)
{
	...
	if (rc) {
		TPD_DMESG( "%s: Failed to set power to state:%d\n",
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (cd->response_buf[HID_OUTPUT_RESPONSE_REPORT_OFFSET]
		...
		TPD_DMESG( "%s: HID output response, wrong report_id\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[4] != HID_OUTPUT_BL_SOP) {
		TPD_DMESG( "%s: HID output response, wrong SOP\n",
			__func__);
		return -EPROTO;
	}

	if (cd->response_buf[size - 1] != HID_OUTPUT_BL_EOP) {
		TPD_DMESG( "%s: HID output response, wrong EOP\n",
			__func__);
		return -EPROTO;
	}
	...
	if (cd->response_buf[size - 3] != LOW_BYTE(crc)
		...
		TPD_DMESG( "%s: HID output response, wrong CRC 0x%X\n",
			__func__, crc);
		return -EPROTO;
	}
	...
	if (status) {
		TPD_DMESG( "%s: HID output response, ERROR:%d\n",
			__func__, status);
		return -EPROTO;
	}
		...
		struct cyttsp5_hid_output *hid_output, bool raw)
{
	...
	if (param_size > 4) {
		TPD_DMESG( "%s: Invalid parameter size\n", __func__);
		return;
	}
	...
	if (cd->response_buf[2] != HID_APP_RESPONSE_REPORT_ID
		...
		TPD_DMESG( "%s: Set Parameter command not successful\n",
			__func__);
		return;
	}
		...
		struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	if (rc == -EPROTO && cd->response_buf[5] == ERROR_COMMAND) {
		...
		*panel_id = PANEL_ID_NOT_ENABLED;
		...
		TPD_DMESG( "%s: Error on Get Panel ID command\n",
			__func__);
		return rc;
	}
		...
		struct cyttsp5_hid_desc *desc)
{
	...
	if (rc) {
		TPD_DMESG( "%s: failed to get HID descriptor length and version, rc=%d\n",
			__func__, rc);
		goto error;
	}
	...
	if (IS_TMO(t)) {
		TPD_DMESG( "%s: HID get descriptor timed out\n",
			__func__);
		rc = -ETIME;
		goto error;
	}
...
static int cyttsp5_hw_soft_reset(struct cyttsp5_core_data *cd)
{
	...
	rc = cyttsp5_hid_cmd_reset_(cd);
	if (rc < 0) {
		TPD_DMESG( "%s: FAILED to execute SOFT reset\n",
				__func__);
		return rc;
	}
...
continue_main_item:
				...
				break;
			default:
				TPD_DMESG( "%s: Unrecognized Main tag %d\n",
					__func__, item_tag);
			}
	...
	if (buf != end) {
		TPD_DMESG( "%s: Report descriptor length invalid\n",
			__func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	if (collection_nest) {
		TPD_DMESG( "%s: Unbalanced collection items (%d)\n",
...
static int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
	...
	if (rc) {
		TPD_DMESG( "%s: Error parsing report descriptor r=%d\n",
			__func__, rc);
	}
...
static int cyttsp5_core_poweroff_device_(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0)
		TPD_DMESG( "%s: HW Power down fails r=%d\n",
				__func__, rc);
	return rc;
}
...
reset:
	...
	if (rc)
		TPD_DMESG( "%s: failed to restore parameters rc=%d\n",
			__func__, rc);
...
static int cyttsp5_core_poweron_device_(struct cyttsp5_core_data *cd)
{
	...
	if (rc < 0) {
		TPD_DMESG( "%s: HW Power up fails r=%d\n", __func__, rc);
		goto exit;
	}
...
reset:
	...
	if (!IS_PIP_VER_GE(&cd->sysinfo, 1, 2)) {
		...
		if (rc)
			TPD_DMESG( "%s: failed to read config version rc=%d\n",
				__func__, rc);
	}

	rc = cyttsp5_get_ic_crc_(cd, CY_TCH_PARM_EBID);
	if (rc)
		TPD_DMESG( "%s: failed to crc data rc=%d\n",
			__func__, rc);
...
static void cyttsp5_startup_work_function(struct work_struct *work)
{
	...
	if (rc < 0)
		TPD_DMESG( "%s: Fail queued startup r=%d\n",
			__func__, rc);
}
...
void tma445a_core_suspend(struct early_suspend *h)
{
	cyttsp5_core_suspend(tp_dev);
}
EXPORT_SYMBOL_GPL(cyttsp5_core_suspend);

void tma445a_core_resume(struct early_suspend *h)
{
	cyttsp5_core_resume(tp_dev);
}
EXPORT_SYMBOL_GPL(tma445a_core_resume);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rc = cyttsp5_startup(cd, true);
	if (rc < 0)
		TPD_DMESG( "%s: HW reset failed r=%d\n",
			__func__, rc);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	case 0:
		if (cd->irq_enabled) {
			...
			/* Disable IRQ */
			mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
			TPD_DMESG( "%s: Driver IRQ now disabled\n",
				__func__);
		} else
			TPD_DMESG( "%s: Driver IRQ already disabled\n",
				__func__);
		break;
	...
	case 1:
		if (cd->irq_enabled == false) {
			...
			/* Enable IRQ */
			mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
			TPD_DMESG( "%s: Driver IRQ now enabled\n",
				__func__);
		} else
			TPD_DMESG( "%s: Driver IRQ already enabled\n",
				__func__);
		break;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch (value) {
	case CY_DBG_SUSPEND:
		TPD_DMESG( "%s: SUSPEND (cd=%p)\n", __func__, cd);
		rc = cyttsp5_core_sleep(cd);
		if (rc)
			TPD_DMESG( "%s: Suspend failed rc=%d\n",
				__func__, rc);
		else
			TPD_DMESG( "%s: Suspend succeeded\n", __func__);
		break;

	case CY_DBG_RESUME:
		TPD_DMESG( "%s: RESUME (cd=%p)\n", __func__, cd);
		rc = cyttsp5_core_wake(cd);
		if (rc)
			TPD_DMESG( "%s: Resume failed rc=%d\n",
				__func__, rc);
		else
			TPD_DMESG( "%s: Resume succeeded\n", __func__);
		break;
	case CY_DBG_SOFT_RESET:
		TPD_DMESG( "%s: SOFT RESET (cd=%p)\n", __func__, cd);
		rc = cyttsp5_hw_soft_reset(cd);
		break;
	case CY_DBG_RESET:
		TPD_DMESG( "%s: HARD RESET (cd=%p)\n", __func__, cd);
		rc = cyttsp5_hw_hard_reset(cd);
		break;
	case CY_DBG_HID_RESET:
		TPD_DMESG( "%s: hid_reset (cd=%p)\n", __func__, cd);
...
undo:
	...
	TPD_DMESG( "%s: failed to create sysfs interface\n", __func__);
	return -ENODEV;
}
...
static int touch_event_handler(void *pdata)
{
	...
	do {
		...
		cyttsp5_irq(CUST_EINT_TOUCH_PANEL_NUM, data);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	int rc = 0;
	//keep device ptr for suspend and resume
	tp_dev = dev;

	if (!pdata || !pdata->core_pdata || !pdata->mt_pdata) {
		TPD_DMESG( "%s: Missing core platform data\n", __func__);
		rc = -ENODEV;
		goto error_no_pdata;
	}
	...
	if (pdata->core_pdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		if (!pdata->core_pdata->power) {
			TPD_DMESG( "%s: Missing platform data function\n",
					__func__);
			rc = -ENODEV;
			goto error_no_pdata;
		}
	...
	if (cd->cpdata->init) {
		...
		TPD_DMESG( "%s: No HW INIT function\n", __func__);
		rc = 0;
	}
	if (rc < 0)
	...
	if (rc < 0) {
		TPD_DMESG( "%s: Error, could not setup IRQ\n", __func__);
	...
	if (rc < 0)
		TPD_DMESG( "%s: Error, device_init_wakeup rc:%d\n",
				__func__, rc);
	...
	if (rc == -ENODEV) {
		TPD_DMESG( "%s: Fail initial startup r=%d\n",
			__func__, rc);
		goto error_startup;
	}
	...
	if (rc < 0) {
		TPD_DMESG( "%s: Error, fail mt probe\n", __func__);
		goto error_startup;
	}

	rc = cyttsp5_btn_probe(dev);
	if (rc < 0) {
		TPD_DMESG( "%s: Error, fail btn probe\n", __func__);
		goto error_startup_mt;
	}

	rc = cyttsp5_proximity_probe(dev);
	if (rc < 0) {
		TPD_DMESG( "%s: Error, fail proximity probe\n", __func__);
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Cypress TrueTouch Core Driver");
