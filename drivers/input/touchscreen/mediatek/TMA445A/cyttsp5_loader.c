		...
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	TPD_DEBUG( "%s: img vers:0x%04X new vers:0x%04X\n", __func__,
			fw_ver_img, fw_ver_new);

	if (fw_ver_new > fw_ver_img) {
		TPD_DEBUG( "%s: Image is newer, will upgrade\n",
				__func__);
		return 1;
	}

	if (fw_ver_new < fw_ver_img) {
		TPD_DEBUG( "%s: Image is older, will NOT upgrade\n",
				__func__);
		return -1;
	}
	...
	TPD_DEBUG( "%s: img revctrl:0x%04X new revctrl:0x%04X\n",
	...
	struct cyttsp5_hex_image *row_image)
{
	...
	if (row_image->row_size > ARRAY_SIZE(row_image->row_data)) {
		TPD_DMESG( "%s: row data buffer overflow\n", __func__);
		rc = -EOVERFLOW;
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
	...
	if (fw_size % image_rec_size != 0) {
		TPD_DMESG( "%s: Firmware image is misaligned\n", __func__);
		rc = -EINVAL;
		goto _cyttsp5_load_app_error;
	}

	TPD_DMESG( "%s: start load app\n", __func__);
	...
	TPD_DMESG( "%s: Send BL Loader Enter\n", __func__);
#ifdef TTHE_TUNER_SUPPORT
	cmd->request_tthe_print(dev, NULL, 0, "Send BL Loader Enter");
#endif
	rc = cyttsp5_ldr_enter_(dev, dev_id);
	if (rc) {
		TPD_DMESG( "%s: Error cannot start Loader (ret=%d)\n",
			__func__, rc);
		goto _cyttsp5_load_app_exit;
	}
	TPD_DEBUG( "%s: dev: silicon id=%08X rev=%02X bl=%08X\n",
	...
	if (rc) {
		TPD_DMESG( "%s: Error cannot init Loader (ret=%d)\n",
			__func__, rc);
		goto _cyttsp5_load_app_exit;
	}

	TPD_DMESG( "%s: Send BL Loader Blocks\n", __func__);
#ifdef TTHE_TUNER_SUPPORT
	...
	while (p < last_row) {
		...
		if (rc) {
			TPD_DMESG( "%s: Program Row Error (array=%d row=%d ret=%d)\n",
				__func__, row_image->array_id,
				row_image->row_num, rc);
			goto _cyttsp5_load_app_exit;
		}

		TPD_DEBUG( "%s: array=%d row_cnt=%d row_num=%04X\n",
			__func__, row_image->array_id, row_count,
			row_image->row_num);
	}

	/* exit loader */
	TPD_DMESG( "%s: Send BL Loader Terminate\n", __func__);
#ifdef TTHE_TUNER_SUPPORT
	cmd->request_tthe_print(dev, NULL, 0, "Send BL Loader Terminate");
#endif
	rc = cyttsp5_ldr_exit_(dev);
	if (rc) {
		TPD_DMESG( "%s: Error on exit Loader (ret=%d)\n",
			__func__, rc);

		/* verify app checksum */
		rc_tmp = cyttsp5_ldr_verify_chksum_(dev);
		if (rc_tmp)
			TPD_DMESG( "%s: ldr_verify_chksum fail r=%d\n",
				__func__, rc_tmp);
		else
			TPD_DMESG( "%s: APP Checksum Verified\n", __func__);
	}
		...
		int fw_size)
{
	...
	cd->wait_for_auto_cali_cplt = false;
	...
	while (retry--) {
		...
		if (rc < 0)
			TPD_DMESG( "%s: Firmware update failed rc=%d, retry:%d\n",
				__func__, rc, retry);
		else
			break;
		msleep(20);
	...
	if (rc < 0) {
		TPD_DMESG( "%s: Firmware update failed with error code %d\n",
			__func__, rc);
		...
		TPD_DEBUG( "%s: Adding callback for calibration\n",
			__func__);
		rc = cmd->subscribe_attention(dev, CY_ATTEN_STARTUP,
			CYTTSP5_LOADER_NAME, cyttsp5_calibration_attention, 0);
		if (rc) {
			TPD_DMESG( "%s: Failed adding callback for calibration\n",
				__func__);
			TPD_DMESG( "%s: No calibration will be performed\n",
				__func__);
			rc = 0;
		} else
	...
	/* CDT214064 */
	cd->wait_for_auto_cali_cplt = true;
		...
		struct cyttsp5_touch_firmware *fw)
{
	...
	if (!ld->si) {
		TPD_DMESG( "%s: No firmware infomation found, device FW may be corrupted\n",
			__func__);
		...
		struct device *dev)
{
	...
	if (panel_id == PANEL_ID_NOT_ENABLED) {
		TPD_DEBUG( "%s: Panel ID not enabled, using legacy firmware\n",
			__func__);
	...
	if (!fws) {
		TPD_DMESG( "%s: No firmwares provided\n", __func__);
		return NULL;
	}
	...
	while ((fw = *fws++)) {
		if (fw->panel_id == panel_id) {
			TPD_DEBUG( "%s: Found matching fw:%p with Panel ID: 0x%02X\n",
				__func__, fw, fw->panel_id);
			return fw;
		}
		TPD_DEBUG( "%s: Found mismatching fw:%p with Panel ID: 0x%02X\n",
			__func__, fw, fw->panel_id);
	}
		...
		bool forced)
{
	...
	if (!ld->loader_pdata) {
		TPD_DMESG( "%s: No loader platform data\n", __func__);
		return rc;
	}

	fw = cyttsp5_get_platform_firmware(dev);
	if (!fw || !fw->img || !fw->size) {
		TPD_DMESG( "%s: No platform firmware\n", __func__);
		return rc;
	}

	if (!fw->ver || !fw->vsize) {
		TPD_DMESG( "%s: No platform firmware version\n",
			__func__);
		return rc;
	}
		...
		void *context)
{
	...
	if (!fw) {
		TPD_DMESG( "%s: No builtin firmware\n", __func__);
		goto _cyttsp5_firmware_cont_builtin_exit;
	}

	if (!fw->data || !fw->size) {
		TPD_DMESG( "%s: Invalid builtin firmware\n", __func__);
		goto _cyttsp5_firmware_cont_builtin_exit;
	}

	TPD_DEBUG( "%s: Found firmware\n", __func__);
...
static int upgrade_firmware_from_class(struct device *dev)
{
	...
	TPD_DEBUG( "%s: Enabling firmware class loader\n", __func__);
	...
	if (retval < 0) {
		TPD_DMESG( "%s: Fail request firmware class file load\n",
			__func__);
		return retval;
	}
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	TPD_DEBUG( "%s: Enabling firmware class loader built-in\n",
		__func__);
		...
		u16 row_number, u16 row_size, u8 *data)
{
	...
	rc = cmd->nonhid_cmd->write_conf_block(dev, 0, row_number,
			...
			&actual_write_len);
	if (rc) {
		TPD_DMESG( "%s: Fail Put EBID=%d row=%d cmd fail r=%d\n",
			__func__, ebid, row_number, rc);
		return rc;
	}

	if (actual_write_len != row_size) {
		TPD_DMESG( "%s: Fail Put EBID=%d row=%d wrong write size=%d\n",
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	TPD_DEBUG( "%s: size:%d row_size=%d row_count=%d\n",
	...
	for (i = 0; i < row_count; i++) {
		TPD_DEBUG( "%s: row=%d size=%d\n", __func__, i, row_size);
		rc = cyttsp5_write_config_row_(dev, ebid, i, row_size,
				row_buf);
		if (rc) {
			TPD_DMESG( "%s: Fail put row=%d r=%d\n",
				__func__, i, rc);
			break;
		}
		row_buf += row_size;
	...
	if (!rc)
		TPD_DEBUG( "%s: TT_CFG updated: rows:%d bytes:%d\n",
	...
	if (rc || verify_crc_status)
		TPD_DMESG( "%s: CRC Failed, ebid=%d, status=%d, scrc=%X ccrc=%X\n",
			__func__, ebid, verify_crc_status,
			calculated_crc, stored_crc);
	else
		TPD_DEBUG( "%s: CRC PASS, ebid=%d, status=%d, scrc=%X ccrc=%X\n",
			__func__, ebid, verify_crc_status,
			calculated_crc, stored_crc);
		...
		const u8 *ttconfig_data, int ttconfig_size)
{
	...
	if (ld->loader_pdata->flags &
		...
		TPD_DEBUG( "%s: img_ver:0x%04X new_ver:0x%04X\n",
			__func__, ld->si->cydata.fw_ver_conf, cfg_ver_new);

		/* Check if config version is newer */
		if (cfg_ver_new > ld->si->cydata.fw_ver_conf) {
			TPD_DEBUG( "%s: Config version newer, will upgrade\n",
			__func__);
			return 1;
		}

		TPD_DEBUG( "%s: Config version is identical or older, will NOT upgrade\n",
			__func__);
	/* Check for config CRC */
		...
		TPD_DEBUG( "%s: img_crc:0x%04X new_crc:0x%04X\n",
			__func__, ld->si->ttconfig.crc, cfg_crc_new);

		if (cfg_crc_new != ld->si->ttconfig.crc) {
			TPD_DEBUG( "%s: Config CRC different, will upgrade\n",
				__func__);
			return 1;
		}

		TPD_DEBUG( "%s: Config CRC equal, will NOT upgrade\n",
			__func__);
	}
		...
		struct device *dev)
{
	...
	if (panel_id == PANEL_ID_NOT_ENABLED) {
		...
		TPD_DMESG( "%s: Panel ID not enabled, using legacy ttconfig\n",
			__func__);
	...
	while ((ttconfig = *ttconfigs++)) {
		if (ttconfig->panel_id == panel_id) {
			/* TODO: Make debug message */
			TPD_DMESG( "%s: Found matching ttconfig:%p with Panel ID: 0x%02X\n",
				__func__, ttconfig, ttconfig->panel_id);
			return ttconfig;
		}
		TPD_DEBUG( "%s: Found mismatching ttconfig:%p with Panel ID: 0x%02X\n",
			__func__, ttconfig, ttconfig->panel_id);
	}
...
static int upgrade_ttconfig_from_platform(struct device *dev)
{
	...
	if (!ttconfig) {
		TPD_DMESG( "%s: No ttconfig data\n", __func__);
		return rc;
	}

	param_regs = ttconfig->param_regs;
	if (!param_regs) {
		TPD_DMESG( "%s: No touch parameters\n", __func__);
		return rc;
	}

	if (!param_regs->data || !param_regs->size) {
		TPD_DMESG( "%s: Invalid touch parameters\n", __func__);
		return rc;
	}

	if (!ttconfig->fw_ver || !ttconfig->fw_vsize) {
		TPD_DMESG( "%s: Invalid FW version for touch parameters\n",
			__func__);
		return rc;
	}
		...
		char *buf, loff_t offset, size_t count)
{
	...
	TPD_DEBUG( "%s: offset:%lld count:%zu\n", __func__, offset, count);
		...
		u8 *bin_config_data, int bin_config_size, u8 **start, int *len)
{
	...
	if (header_size < 11 || header_size >= bin_config_size - 6) {
		TPD_DMESG( "%s: Invalid header size %d\n", __func__,
			header_size);
		return -EINVAL;
	}
	...
	if (config_size != bin_config_size - header_size - 2) {
		TPD_DMESG( "%s: Config size invalid\n", __func__);
		return -EINVAL;
	}
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if (!ld->si)
		TPD_DMESG( "%s: Fail get sysinfo pointer from core\n",
			__func__);
#if !CYTTSP5_FW_UPGRADE
	TPD_DMESG( "%s: No FW upgrade method selected!\n", __func__);
#endif
...
static int cyttsp5_loader_probe(struct device *dev, void **data)
{
	...
	if (!pdata || !pdata->loader_pdata) {
		TPD_DMESG( "%s: Missing platform data\n", __func__);
		rc = -ENODEV;
		goto error_no_pdata;
	}
	...
	if (rc) {
		TPD_DMESG( "%s: Error, could not create forced_upgrade\n",
				__func__);
	...
	if (rc) {
		TPD_DMESG( "%s: Error, could not create manual_upgrade\n",
				__func__);
	...
	if (rc) {
		TPD_DMESG( "%s: Error, could not create config_loading\n",
				__func__);
	...
	if (rc) {
		TPD_DMESG( "%s: Error, could not create config_data\n",
				__func__);
	...
	*data = ld;
	...
	TPD_DMESG( "%s: Successful probe %s\n", __func__, dev_name(dev));
	return 0;
