...
{
	ULONG len = count;
	pr_warn(KERN_INFO DFT_TAG "write parameter len = %d\n\r", (int)len);
...
static void stp_sdio_txdbg_dump(void)
{
	...
	for (i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
...
#endif
		pr_warn(KERN_INFO DFT_TAG "pkt_buf.tx_buf idx(%x) ts(%d) len(%d)\n", idx,
		...
		pr_warn(KERN_INFO DFT_TAG "pkt_buf.tx_buf dump ok\n");
	}
