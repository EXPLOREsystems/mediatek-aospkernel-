...
void ccci_aed(int md_id, unsigned int dump_flag, char *aed_str)
{
  ...
  #ifdef CONFIG_MTK_WEAR_SUPPORT_MODEM /* Jack temp modification */
	#if defined(CONFIG_MTK_AEE_FEATURE) && defined(ENABLE_AEE_MD_EE)
						  ...
						  modem_runtime_info_tag_t *tag)
{
	...
	{
		runtime->IPCShareMemBase = ctl_b->smem_table->ccci_ipc_smem_base_phy+offset_of(CCCI_IPC_MEM, buffer) - md_2_ap_phy_addr_offset_fixed;
