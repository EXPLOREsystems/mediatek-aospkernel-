...
static int create_files(struct super_block *sb, struct dentry *root)
{
	...
	for (event = FIRST_ACTIVITY_EVENT; event <= LAST_HW_COUNTER; event++) {
		...
		case COUNTER_FP3_C1:
			...
			break;
		default:
			pr_debug("gator: trying to create file for non-existent counter (%d)\n", event);
			continue;
		}
...
static void mali_counter_initialize(void)
{
	...
	if (mali_set_hw_event) {
		...
		pr_debug("gator: mali online _mali_profiling_set_event symbol not found\n");
	}
	...
	if (mali_set_fb_event) {
		...
		pr_debug("gator: mali online _mali_osk_fb_control_set symbol not found\n");
	}
	...
	if (mali_control) {
		...
		pr_debug("gator: mali online _mali_profiling_control symbol not found\n");
	}
...
static void mali_counter_deinitialize(void)
{
	...
	if (mali_set_hw_event) {
		...
		pr_debug("gator: mali offline _mali_profiling_set_event symbol not found\n");
	}
	...
	if (mali_set_fb_event) {
		...
		pr_debug("gator: mali offline _mali_osk_fb_control_set symbol not found\n");
	}
	...
	if (mali_control) {
		...
		pr_debug("gator: mali offline _mali_profiling_control symbol not found\n");
	}
...
static int start(void)
{
	...
	if (GATOR_REGISTER_TRACE(mali_sw_counter)) {
		pr_debug("gator: mali_sw_counter tracepoint failed to activate\n");
		return -1;
	}
#elif GATOR_MALI_INTERFACE_STYLE == 3
/* For Mali drivers with built-in support. */
	if (GATOR_REGISTER_TRACE(mali_sw_counters)) {
		pr_debug("gator: mali_sw_counters tracepoint failed to activate\n");
		return -1;
	}
#else
