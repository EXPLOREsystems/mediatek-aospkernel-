...
static void msdc_dump_register(struct msdc_host *host)
{
    ...
    if (host->id == 0) {
        ...
        INIT_MSG("MSDC0 CLK,CMD,DAT0~1 PINMUX mode %x", sdr_read32(MSDC0_CLK_PINMUX_ADDR));
	INIT_MSG("MSDC0 TDSEL %x", sdr_read32(MSDC0_PAD_CTRL_TDSEL_CFG));
	INIT_MSG("MSDC0 RDSEL %x", sdr_read32(MSDC0_PAD_CTRL_RDSEL_CFG));
        INIT_MSG("MSDC0 DAT2~7,RST PINMUX mode %x", sdr_read32(MSDC0_DAT2_PINMUX_ADDR));
    } else if (host->id == 1) {
	...
	INIT_MSG("MSDC1 RDSEL %x", sdr_read32(MSDC1_PAD_CTRL_RDSEL_CFG));
        INIT_MSG("MSDC1 CLK,CMD,DAT0~3 PINMUX mode %x", sdr_read32(MSDC1_CMD_PINMUX_ADDR));
        INIT_MSG("MSDC1 DAT3 PINMUX mode %x", sdr_read32(MSDC1_DAT3_PINMUX_ADDR));
    }
...
{
#if 0
/* Comment out since new Combo eMMC does not need it
/*
    u64 device_capacity = 0;
				      ...
				      unsigned long       timeout)
{
	...
	*/
    ...
    #ifdef MSDC1_SDIO_UT
    if (sd_debug_zone[host->id]&DBG_EVT_CMD) printk(KERN_ERR "CMD<%d><0x%.8x> Arg<0x%.8x>", opcode , rawcmd, rawarg);
    #else
...
static int ot_command_resp_polling(struct msdc_host *host, struct mmc_command *cmd, int tune, unsigned long timeout, u32 *pintsts)
{
    ...
    u32 acmd53done = MSDC_INT_ACMDTMO | MSDC_INT_DATTMO | MSDC_INT_CMDRDY | MSDC_INT_CMDTMO | MSDC_INT_RSPCRCERR;
    u32 cmdsts = MSDC_INT_CMDTMO | MSDC_INT_ACMDTMO | MSDC_INT_DATTMO | MSDC_INT_ACMDCRCERR | MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR;
    ...
    #if 0 /* FIX ME: HW does not support */
    /* command interrupts */
...
static u32 wints_cmd_testmode = MSDC_INT_CMDRDY | MSDC_INT_RSPCRCERR | MSDC_INT_CMDTMO |
		       ...
		       MSDC_INT_DATTMO |
		       MSDC_INT_ACMDCRCERR | MSDC_INT_DATCRCERR;
...
static unsigned int ot_init(struct msdc_host *host, struct ot_data *potData)
{
    ...
    sdr_set_field(MSDC_IOCON, MSDC_IOCON_DSPLSEL, 1);
    sdr_set_field(MSDC_IOCON, MSDC_IOCON_W_DSPLSEL, 1);
...
static inline int emmc_proc_info(struct seq_file *m, struct hd_struct *this)
{
    ...
    return seq_printf(m, "emmc_p%d: %8.8x %8.8x \"%s\"\n", this->partno,
	       ...
	       (char *)(this->info ? this->info->volname : no_partition_name));
}
#else
static inline int emmc_proc_info(struct seq_file *m, struct hd_struct *this)
{
