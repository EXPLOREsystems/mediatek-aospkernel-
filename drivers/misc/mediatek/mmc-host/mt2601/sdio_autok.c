...
int autok_io_rw_extended(struct msdc_host *host, unsigned int u4Addr, unsigned int u4Func, void *pBuffer, unsigned int u4Len, bool write)
{
    ...
    #if defined(SDIO_UT_WITHOUT_6630_DRIVER)
    max_blocks = 511u;
    #else
...
unsigned int autok_get_current_vcore_offset(void)
{
	...
	if (vproc_vosel_ctrl == 0)
	    pmic_read_interface(0x21E, &vcore_sel, 0x7F, 0x0);
	else if (vproc_vosel_ctrl == 1)
	    pmic_read_interface(0x220, &vcore_sel, 0x7F, 0x0);
    #else
	/* vcore_sel = mt6333_get_reg_value(0x6C); // PMIC MT6333 */
    #endif
...
static void autok_tuning_parameter_init(struct msdc_host *host, E_AUTOK_TUNING_STAGE stg, U_AUTOK_INTERFACE_DATA *pAutokData)
{
	...
	if (freq_mhz <= AUTOK_THRESHOLD_FREQ)
...
static void autok_vcore_set(unsigned int vcore_uv)
{
	...
	/* pmic_read_interface(0x220, &vcore_sel, 0x7F, 0x0); //Debug purpose */
	/* pmic_read_interface(0x21e, &vcore_sel, 0x7F, 0x0); //Debug purpose */
    #elif defined(USE_VCORE_DVFS_API)
