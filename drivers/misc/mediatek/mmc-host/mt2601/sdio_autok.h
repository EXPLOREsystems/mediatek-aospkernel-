...
#if defined(USE_CPUFREQ_API) && defined(USE_VCORE_DVFS_API)
#error USE_CPUFREQ_API and USE_VCORE_DVFS_API cannot be both defined
#endif
#if !defined(USE_CPUFREQ_API) && !defined(USE_VCORE_DVFS_API)
#error USE_CPUFREQ_API and USE_VCORE_DVFS_API cannot be both un-defined
#endif
...
extern unsigned int mt6333_get_reg_value(unsigned int reg);
...
#if !defined(SDIO_AUTOK_UT_NO_VOLTAGE_DEP)
    ...
    #elif defined(USE_VCORE_DVFS_API)
...
#else
    #if defined(USE_CPUFREQ_API)
    #define mt_cpufreq_disable(unsigned int type, bool disabled)
    #define mt_cpufreq_volt_set_by_sdio(unsigned int volt)
    #else
    #define mt_vcore_dvfs_disable_by_sdio(type, disabled)
    #define mt_vcore_dvfs_volt_set_by_sdio(volt)
    #define mt_vcore_dvfs_volt_get_by_sdio()    1000000
    #endif
#endif
/* #endif */
