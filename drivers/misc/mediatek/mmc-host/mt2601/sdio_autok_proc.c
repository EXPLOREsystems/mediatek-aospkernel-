...
static int autok_thread_func(void *data)
{
    ...
    if (t1.tv_usec < t0.tv_usec) {
        time_in_s -= 1;
	time_in_ms = (1000000 + t1.tv_usec - t0.tv_usec)>>10;
    } else {
			...
			const char *buf, size_t count)
{
	...
	case PARAMS:
	    ...
	    printk(KERN_ERR "[AUTOKD] p_single_autok[%d].vol_count=%d, p_single_autok[%d].param_count=%d\n",
		id, p_single_autok[id].vol_count, p_single_autok[id].param_count);
            for (i = 0; i < p_single_autok[id].vol_count; i++) {
		printk(KERN_ERR "[AUTOKD] p_single_autok[%d].vol_list[%d]=%d\n", id, i, p_single_autok[id].vol_list[i]);
	    }
            for (i = 0; i < p_single_autok[id].vol_count; i++) {
