...
void u3d_init(void)
{
	...
	pr_debug("u3d_initialize_drv\n");
	u3d_initialize_drv();
#if (BUS_MODE == QMU_MODE)
	pr_debug("mu3d_hal_init_qmu\n");
	/* Initialize QMU module. */
	mu3d_hal_init_qmu();
	pr_debug("mu3d_hal_init_qmu end\n");
#endif
		    ...
		    DEV_INT8 en_u2)
{
	...
	pr_debug("mode : 0x%08X\n", mode);
	pr_debug("u1_value : 0x%08X\n", u1_value);
	pr_debug("u2_value : 0x%08X\n", u2_value);
	pr_debug("en_u1 : 0x%08X\n", en_u1);
	pr_debug("en_u2 : 0x%08X\n", en_u2);
		...
		if ((os_readl(U3D_EP0CSR) & EP0_SETUPPKTRDY)	/* SETUPEND case; restarts from EP0_IDLE */
			...
			if (((Request->bmRequestType & USB_TYPE_MASK) == USB_TYPE_STANDARD) &&
				...
				if (Request->bmRequestType & USB_DIR_IN) {
					...
					if ((Request->bRequest == USB_REQ_CLEAR_FEATURE) &&
					    (Request->wValue == ENDPOINT_HALT)) {
						pr_debug("==USB_REQ_CLEAR_FEATURE==\n");
						u3d_clear_stall_all();
						g_device_halt = 0;
						os_writel(U3D_EP0CSR,
							  os_readl(U3D_EP0CSR) | EP0_SETUPPKTRDY |
