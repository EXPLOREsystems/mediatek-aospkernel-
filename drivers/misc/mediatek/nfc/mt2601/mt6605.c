...
/* #include <mach/mt6605.h> // vsim chage */
...
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    /*NRES_gpio , AS39230 RST pin */
    #if 0
    mt_set_gpio_mode(platform_data->nres_gpio   , GPIO_NFC_FIRM_PIN_M_GPIO); /* Set gpio mode */
    mt_set_gpio_dir(platform_data->nres_gpio    , GPIO_DIR_OUT);  /* Set Dir =  output */
    mt_set_gpio_out(platform_data->nres_gpio    , GPIO_OUT_ZERO); /* Set output Low */
    #endif
...
static ssize_t mt6605_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	printk(KERN_DEBUG "%s : wait_event_interruptible ret,%d, IRQ,%d\n",
		__func__,
		ret,
		mt_get_gpio_in(mt6605_dev->irq_gpio));
...
static ssize_t mt6605_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
    ...
    #if 0
    {
	/* mt_set_gpio_mode( mt6605_dev->irq_gpio        , GPIO_IRQ_NFC_PIN_M_EINT); // Set gpio mode */
	/* mt_set_gpio_dir( mt6605_dev->irq_gpio         , GPIO_DIR_IN);  // Set Dir =  input */
	/* mt_set_gpio_pull_enable( mt6605_dev->irq_gpio , GPIO_PULL_ENABLE); */
	/* mt_set_gpio_pull_select( mt6605_dev->irq_gpio , GPIO_PULL_DOWN); */
	msleep(100);
        printk(KERN_DEBUG "%s : IRQ,%d, mode %d \n", __func__,
		mt_get_gpio_in(mt6605_dev->irq_gpio),
		mt_get_gpio_mode(mt6605_dev->irq_gpio));
	printk(KERN_DEBUG "%s : GPIO40,%d, GPIO52,%d, GPIO84,%d, GPIO89,%d, GPIO100,%d .(CUST_EINT_POLARITY_HIGH)\n", __func__,
		mt_get_gpio_mode(GPIO40),
		mt_get_gpio_mode(GPIO52),
		mt_get_gpio_mode(GPIO84),
		mt_get_gpio_mode(GPIO89),
		mt_get_gpio_mode(GPIO100)
	    );
...
static long mt6605_dev_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,0x%X,0x%X.\n", tmp_gpio, gpio_num);
	...
	if (cmd == MTK_NFC_IOCTL_READ) { /* READ */
		...
		if (gpio_pol == MTK_NFC_PULL_LOW) {
		    ...
		    /* --- for AS39230 RST pin control */
		    #if 0
		    printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,0x%X,0x%X.\n", tmp_gpio, MTK_NFC_GPIO_SYSRST_B);
		    if (tmp_gpio == MTK_NFC_GPIO_SYSRST_B) {
			result = mt_set_gpio_out(mt6605_dev->nres_gpio, GPIO_OUT_ZERO);
			printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,GPIO_NFC_FIRM_PIN,Low.\n");
		    } else {
			printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,GPIO_NFC_FIRM_PIN, else case!\n");
		    }
		    #endif
		    /* --- */
		    ...
		    printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,0x%X,0x%X.\n", tmp_gpio, MTK_NFC_GPIO_SYSRST_B);
		    if (tmp_gpio == MTK_NFC_GPIO_SYSRST_B) {
			result = mt_set_gpio_out(mt6605_dev->nres_gpio, GPIO_OUT_ONE);
			printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,GPIO_NFC_FIRM_PIN,High.\n");
		    } else {
			printk(KERN_DEBUG "mt6605_dev_unlocked_ioctl,GPIO_NFC_FIRM_PIN, else case !\n");
		    }
		    #endif
