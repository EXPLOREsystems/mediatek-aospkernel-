...
int eMMC_rw_x(loff_t addr, u32  *buffer, int host_num, int iswrite, u32 totalsize, int transtype, Region part)
{
	...
	int result = 0;
	if (addr < 0) {
		env_err("DumChar ERROR:Wrong Address %llx for emmc!\n", addr);
		return -EINVAL;
	}
	...
	cmd.address = addr/512;
		...
		env_err("DumChar ERROR: Wrong Address\n");
		return -EINVAL;
	}
...
static void load_default_env(void)
{
	...
	if (tmp == NULL) {
		env_err("[%s]can not find %s,set the default value\n", MODULE_NAME, DATA_FREE_SIZE_TH_NAME);
...
static void env_init(void)
{
	...
	if (!env_buffer) {
		env_err("malloc env buffer fail\n");
		return;
	}
	...
	if (ret < 0) {
		env_err("[%s]read_env_area fail, ret = %x\n", MODULE_NAME, ret);
		env_valid = 0;
		goto end;
	}
	...
	if (!strcmp(g_env.sig, ENV_SIG) && !strcmp(g_env.sig_1, ENV_SIG)) {
		...
		if (checksum != g_env.checksum) {
			env_err("[%s]checksum mismatch s %d d %d!\n", MODULE_NAME, g_env.checksum, checksum);
			env_valid = 0;
			goto end;
		} else{
			env_info("[%s]ENV initialize sucess\n", MODULE_NAME);
			env_valid = 1;
		}
	} else {
		env_err("[%s]ENV SIG Wrong\n", MODULE_NAME);
		env_valid = 0;
		goto end;
	}
...
add:
	...
	if (len > (&env_data[CFG_ENV_DATA_SIZE]-env)) {
		env_err("## Error: environment overflow, \"%s\" deleted\n", name);
		return -1;
	}
	while ((*env = *name++) != '\0')
		env++;
...
static ssize_t env_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *ppos)
{
	...
	if (buffer == NULL) {
	ret = -ENOMEM;
		env_err("[env_proc_write]alloc buffer fail\n");
		goto fail_malloc;
	}
	...
	if (i == size) {
		env_err("write fail, buffer:%s\n", buffer);
		ret = -EFAULT;
		goto end;
	} else {
		env_info("[env_proc_write]name :%s,value:%s\n", buffer, buffer+v_index);
	}
	...
	if (!strcmp(buffer, DATA_FREE_SIZE_TH_NAME)) {
		...
		if (value_new >= data_free_size) {
			env_err("new value %llx more than data free size %llx, setting fail\n", value_new, data_free_size);
			ret = -EFAULT;
			goto end;
		}
	#else
		env_err("[env_proc_write]it don't support limit sdcard size\n");
		ret = -EFAULT;
		goto end;
	#endif
	}
