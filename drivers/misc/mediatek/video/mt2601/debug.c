...
void DBG_OnLcdDone(void)
{
	...
	printk("[DBG] Touch Latency: %ld ms\n",
	       (time - tpd_last_down_time) / 1000);

	printk("[DBG] LCD update time %ld ms (TE delay %ld ms + LCD %ld ms)\n",
	...
	printk("[DBG] MTKFB FPS: %ld.%02ld, Avg. update time: %ld.%02ld ms "
...
void DBG_OnHDMIDone(void)
{
	...
	printk("[DBG][HDMI] FPS: %ld.%02ld, Avg. update time: %ld.%02ld ms\n",
	       f / 100, f % 100,
	       update / 100, update % 100);
...
static void process_dbg_opt(const char *opt)
{
	...
	if (addr) {
	    printk("[DBG] Read register 0x%08x: 0x%08x\n", addr, INREG32(addr));
	} else {
	    goto Error;
	}
    }
	...
	printk("[DBG] process_dbg_opt(), set backlight level = %d\n", level);
	...
	printk("[DBG] process_dbg_opt(), %d %d %d %d %d %d\n", lrs, lgs, lbs, dbr, dbg, dbb);
    }
...
static ssize_t layer_debug_open(struct inode *inode, struct file *file)
{
	...
	printk("[DBG] Vmalloc to get temp buffer failed\n");
			   ...
			   const char __user *ubuf, size_t count, loff_t *ppos)
{
    ...
    printk("[DBG] mtkfb_layer%d write is not implemented yet\n", dbgopt->layer_index);
