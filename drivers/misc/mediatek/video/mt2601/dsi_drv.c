...
static volatile bool isTeSetting;
static volatile bool dsiStartTransfer;
static volatile bool dsiTeEnable = true;
static volatile bool dsiTeExtEnable;
...
int fbconfig_get_dsi_ssc(void)
{
    if (fbconfig_dsi_ssc == 0)
        return ((lcm_params->dsi.ssc_range) ? lcm_params->dsi.ssc_range : 4);
    else
