...
LCD_STATUS LCD_Set_DrivingCurrent(LCM_PARAMS *lcm_params)
{
	...
	if (GPIO_46_MODE == GPIO_MODE_03) /* LPD_MX */
	{
	    switch (lcm_params->dbi.ctrl_io_driving_current)
	    {
		    ...
		    MASKREG32(LCD_IODRV_REG_CTRL_MX, 0x000000C7, 0x00000041);
		    printk("[WARNING] Driving current settings incorrect!\n");
		    printk("dbi.io_driving_current must be 2 mA / 4 mA / 6 mA / 8 mA / 10 mA / 12 mA / 14 mA / 16 mA.\n");
	...
	else
	{
	    switch (lcm_params->dbi.ctrl_io_driving_current)
	    {
		    ...
		    MASKREG32(LCD_IODRV_REG_CTRL, 0x00000007, 0x00000001);
		    printk("[WARNING] Driving current settings incorrect!\n");
		    printk("dbi.io_driving_current must be 4 mA / 8 mA / 12 mA / 16 mA / 20 mA / 24 mA / 28 mA / 32 mA.\n");
		    printk("Set to default (8 mA).\n");
		    break;
	    }
	}
