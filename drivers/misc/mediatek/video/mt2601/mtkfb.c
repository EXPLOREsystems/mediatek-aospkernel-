...
unsigned int display_power_state = 0;
...
static void mtkfb_update_screen_impl(void)
{
    ...
    if (is_early_suspended)
	DISP_CHECK_RET(DISP_UpdateScreenEarlySuspend(0, 0, 0, fb_xres_update, fb_yres_update));
    else
...
static int mtkfb_pan_display_proxy(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    printk("[AppLaunch] mtkfb_pan_display_proxy.\n");
#endif
...
static int mtkfb_set_multiple_layers_early_suspend(struct fb_info *info, struct fb_overlay_layer *layerInfo)
{
    int i, layerId;
    int is_update = 0;
    ...
    if (down_interruptible(&sem_early_suspend)) {
	printk("[FB Driver] can't get semaphore in mtkfb_set_multiple_layers_early_suspend()\n");
	...
	if (is_lcm_always_on)
	{
		...
		if (layerInfo[i].layer_enable)
		    realtime_layer_config[i].buff_idx = layerInfo[i].next_buff_idx;
		realtime_layer_config[i].layer_en = layerInfo[i].layer_enable;
		captured_layer_config[i].isDirty = FALSE;
	    }
	    mutex_unlock(&OverlaySettingMutex);
	    DISP_UpdateScreenEarlySuspend(1, 0, 0, fb_xres_update, fb_yres_update);
	}
	is_update = 1;
    }
...
#endif
{
	...
	else
	{
	    ...
	    if (mtkfb_set_multiple_layers_early_suspend(info, layerInfo))
		return (r);
	...
	else
	{
		...
		else
		{
		    mutex_lock(&ScreenCaptureMutex);
		    if (down_interruptible(&sem_early_suspend)) {
			MTKFB_WRAN("[FB Driver] can't get semaphore in MTKFB_PREPARE_OVERLAY_BUFFER\n");
			mutex_unlock(&ScreenCaptureMutex);
			return -EFAULT;
		    }
		    sem_early_suspend_cnt--;
...
/* /================ */
	...
	case DISP_IOCTL_TRIGGER_SESSION:
	{
		...
		if ((DISP_SESSION_TYPE(config.session_id) == DISP_SESSION_PRIMARY) && (config.present_fence_idx != 0))
			gPresentFenceIndex = config.present_fence_idx;
...
/* ////////////////////////////////////////////// */
	...
	else
	{
	    ...
	    printk("[FB]: MTKFB_LCM_ALWAYS_ON_ENABLE: %d %d\n", enable, is_lcm_always_on);
	}
	...
	else
	{
	    ...
	    printk("[FB]: MTKFB_SET_DISPLAY_POWER_MODE: %d\n", display_power_state);
	}
...
static void mtkfb_late_resume(struct early_suspend *h)
{
	...
	if (is_lcm_always_on)
	{
	    ...
	    for (i = 0; i < DDP_OVL_LAYER_MUN; i++)
	    {
		if (isAEEEnabled && (i == ASSERT_LAYER) && (cached_layer_config[i].addr == dal_fb_pa))
		{
		    cached_layer_config[i].layer_en = TRUE;
